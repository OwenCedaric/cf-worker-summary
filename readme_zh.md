...
- 支持允许域名的网络文章摘要化
- **原生文档支持**：直接摘要 **PDF, Word, Excel, PowerPoint** 等多种文档格式
...
- **混合内容提取**：结合 Jina Reader 与 Cloudflare Workers AI，兼顾成功率与准确度
- 多语言自动检测与生成
- 自动截断长文本，支持自定义提示模板
...
Worker 采用混合模式以确保最高质量的内容提取：
1.  **原生文档检测**：如果 URL 指向文档（如 PDF, Docx），直接使用 Cloudflare Workers AI 的 `toMarkdown()` 进行处理。
2.  **Jina Reader (首选)**：对于普通网页，默认使用 [Jina Reader](https://r.jina.ai) 以利用其强大的反爬虫和正文清洗能力。
3.  **Cloudflare 后备**：如果 Jina Reader 请求失败，系统会回退到直接抓取并使用 Cloudflare 的 `toMarkdown()` 进行高质量的 HTML 内容提取。
